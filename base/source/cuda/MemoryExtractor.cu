...
__global__ void extractKernel<Bit>(const Bit::Word* source,
                                   ...
                                   const unsigned int* indices,
                                   unsigned int num_indices) {
  // TODO(rvhoang): This needs shared mem
  unsigned int index = blockDim.x * blockIdx.x + threadIdx.x;
  unsigned int stride = blockDim.x * gridDim.x;
  ...
  unsigned int limit = ((num_indices + 31) / 32) * 32;
  while (index < limit) {
    warp_result = 0;
...
void extract(const typename Storage<T>::type* source,
  ...
  unsigned int blocks_per_grid =
    (num_indices + threads_per_block - 1) / threads_per_block;
  extractKernel<T><<<blocks_per_grid,
                     threads_per_block,
                     0,
...
template void extract<float>(const typename Storage<float>::type* source,
                        ...
                        const unsigned int* indices,
                        unsigned int num_indices);
template void extract<Bit>(const typename Storage<Bit>::type* source,
